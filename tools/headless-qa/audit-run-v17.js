// Bot Ecosystem V17 -- Staff Market / Hiring Economy Root-Cause Audit. Baut
// 1:1 auf audit-run-v16.js auf (identisches Setup/Cohorts/Snapshots) --
// Ergaenzung: qaTraceAllStaffHireOptions() (V17, neu) -- vollstaendige
// Pro-Rolle-Diagnose JEDER aktuell vakanten Personal-Rolle mit exaktem,
// primaerem Reason-Code (MARKET_EMPTY/NO_AFFORDABLE_CANDIDATE/BUDGET_GATE/
// RESERVE_GATE/ROLE_PRIORITY_GATE/NONE_HIRE_STAFF_CREATED) + Dump von
// qaStaffHireTraceLog. Alle V13-V16-Fixes bleiben unveraendert (Feature
// Freeze) -- dieser Lauf reproduziert V16-Post-Fix als V17-Baseline.
// Nutzung: node audit-run-v17.js <seed> <targetSeasons> <outFile>
const path = require('path');
const fs = require('fs');
const { createHeadlessGame } = require('./headless-runner');

const SEED = process.argv[2] || 'V17-A';
const TARGET_SEASONS = parseInt(process.argv[3] || '50', 10);
const OUT_FILE = process.argv[4] || path.join(__dirname, '_auditv17_' + SEED + '.json');
...
function poolHealthSnapshotFn(seasonNum) {
  return { season: seasonNum, roles: qaCaptureStaffPoolHealth() };
}
...
function finalDumpFn() {
  return {
    ...
    staffHireTraceLog: (typeof qaStaffHireTraceLog !== 'undefined') ? qaStaffHireTraceLog : [],
  };
}
...
(async () => {
  ...
  const out = {
    ...
    powerSnapshots: [], staffSnapshots: [], distributionSnapshots: [], marketPoolSnapshots: [], poolHealthSnapshots: [],
    generationLog: [], retirementLog: [], rawPotentialLog: [], selfHealLog: [], transferLog: [], scrimLog: [], championLog: [], decisionLog: [], missedOpportunityLog: [], staffVacancyLog: [], openVacanciesAtEnd: {}, staffHireTraceLog: [],
    errors: [],
  };
  ...
  out.poolHealthSnapshots.push(game.run(poolHealthSnapshotFn, 0));
  flush();
  ...
  for (let season = 1; season <= TARGET_SEASONS; season++) {
    ...
    game.run(() => { qaScanAndUpdatePeaks(); qaScanStaffVacancies(); qaTraceAllStaffHireOptions(); });

    out.powerSnapshots.push(game.run(powerSnapshotFn, season));
    out.marketPoolSnapshots.push(game.run(marketPoolSnapshotFn, season));
    out.poolHealthSnapshots.push(game.run(poolHealthSnapshotFn, season));
  ...
  out.staffHireTraceLog = dump.staffHireTraceLog;

  out.durationMs = Date.now() - t0all;
  out.pageErrors = game.pageErrors;
  flush();
  console.log('[' + SEED + '] AUDIT V17 DONE in', out.durationMs, 'ms. pageErrors:', game.pageErrors.length,
    'decisionLog:', out.decisionLog.length, 'staffVacancyLog:', out.staffVacancyLog.length,
    'staffHireTraceLog:', out.staffHireTraceLog.length, 'powerSnapshots:', out.powerSnapshots.length);
})().catch((err) => { console.error('[' + SEED + '] AUDIT V17 FAILED:', err); process.exit(1); });
