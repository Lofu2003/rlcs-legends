...
console.log('\n=== PHASE 27: Champion Diversity (nur V14-BASE-F -- einziger Seed mit Champion-Log) ===');
{
  const f = runs.find((r) => r.seed === 'F').data;
  const champLog = f.championLog || [];
  if (!champLog.length) {
    console.log('Kein championLog in diesem Lauf gefunden -- Instrumentierung/Datenproblem, siehe Abschlussbericht.');
  } else {
    const worlds = champLog.filter((c) => c.eventType === 'worlds').sort((a, b) => a.season - b.season);
    const majors = champLog.filter((c) => c.eventType === 'major').sort((a, b) => a.season - b.season);
    console.log('n Worlds-Eintraege=' + worlds.length + ' n Major-Eintraege=' + majors.length);
    [10, 20, 50].forEach((horizon) => {
      const w = worlds.filter((c) => c.season <= horizon);
      const distinct = new Set(w.map((c) => c.championName));
      console.log('Worlds S1-S' + horizon + ': ' + w.length + ' Titel, ' + distinct.size + ' verschiedene Champions (' + round(100 * distinct.size / (w.length || 1), 1) + '% Diversitaet)');
    });
    console.log('Worlds-Champions chronologisch: ' + worlds.map((w) => 'S' + w.season + '=' + w.championName).join(', '));
  }
}
